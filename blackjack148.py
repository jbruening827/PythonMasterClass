...
def load_images(card_images):
    ...
    for suit in suits:
        ...
        for card in range(1, 11):
            name = "/Users/justinbruening/Python Masterclass Udemy/ModulesandFunctions/cards/{}_{}.{}".format(str(card), suit, extension)
...
def score_hand(hand):
    ...
    for next_card in hand:
        ...
        if card_value == 1 and not ace:
            ...
            card_value = 11
        score += card_value
        # if we would bust, then check if there's an ace and subtract 10 if there is
        if score > 21 and ace:
            score -= 10
            ace = False
    return score
...
def initial_deal():
    ...
    dealer_hand.append(_deal_card(dealer_card_frame))    # deals first card for dealer
    dealer_score_label.set(score_hand(dealer_hand))
    deal_player()           # deals card for player again
...
# buttons connected to deal functions
